...
function normalizeRequest(value: unknown): AIQuestionRequest | null {
  ...
  return { question, previousResponseId };
}
...
export async function handleChatRequest(request: Request) {
  ...
  if (!lease.allowed) {
    return errorResponse(
      "rate_limited",
      "Too many questions were sent recently. Please try again later.",
      429,
      ...
      try {
        ...
        for await (const event of responseStream) {
          if (event.type === "response.output_text.delta") {
            if (!safeEnqueue(controller, { type: "delta", text: event.delta })) {
              break;
            }
